    ...
    <div class="col-lg-11 mb-9 mx-auto">
        ...
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <a href="{{ route('kaskeluar.create') }}" class="btn btn-primary float-right">
            ...
            <div class="card-body">
                ...
                <table id="dataTable" class="table table-bordered" cellspacing="1"><br />
                    <thead>
                        <tr align="center">
                            ...
                            <th style="width: 10%">No Kas Keluar</th>
                    ...
                    <tbody>
                        @foreach ($kaskeluar as $transaksi)
                            <tr align="center">
                                <td>{{ $transaksi->tgl }}</td>
                                <td>{{ $transaksi->no_kaskeluar }}</td>
                                <td>{{ $transaksi->deskripsi }}</td>
                                <td>Rp. {{ number_format($transaksi->jumlah, 2, ',', '.') }}</td>
                                <td>
                                    <a href="{{ route('kaskeluar.show', $transaksi->id) }}" class="btn btn-sm btn-warning">
                            ...
                            <div class="modal fade" id="confirmDeleteModal{{ $transaksi->id }}" tabindex="-1"
                                ...
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        <form method="post" action="{{ route('kaskeluar.destroy', $transaksi->id) }}">
                                            @csrf
                                            @method('delete')
                ...
                <div class="text-right mb-3">
                    <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#confirmResetModal">
                        <i class="fas fa-trash"></i> Reset Seluruh Isi Kas Keluar
                    </button>
                </div>
                ...
                <div class="modal fade" id="confirmResetModal" tabindex="-1" role="dialog"
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <form method="post" action="{{ route('kaskeluar.reset') }}">
                                @csrf
                                <div class="modal-header">
